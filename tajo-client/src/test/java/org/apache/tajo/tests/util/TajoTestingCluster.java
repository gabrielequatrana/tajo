...
public class TajoTestingCluster {
	...
	////////////////////////////////////////////////////////
	private void startMiniTajoCluster(File testBuildDir, final int numSlaves, boolean local) throws Exception {
		...
		if (!local) {
			...
			FileTablespace defaultTableSpace = new FileTablespace(TablespaceManager.DEFAULT_TABLESPACE_NAME,
					defaultTsUri, null);
	...
	private void setupCatalogForTesting(TajoConf c, File testBuildDir) throws IOException, UnsupportedCatalogStore {
		...
		if (hiveCatalogClassExists && driverClass != null && driverClass.equals(HIVE_CATALOG_CLASS_NAME)) {
			try {
				getClass().getClassLoader().loadClass(HIVE_CATALOG_CLASS_NAME);
	...
	public static ResultSet run(String[] names, Schema[] schemas, String[][] tables, String query, TajoClient client)
			throws Exception {
	...
	public static ResultSet run(String[] names, Schema[] schemas, String[][] tables, String query) throws Exception {
	...
	public static void createTable(TajoConf conf, String tableName, Schema schema, String[] tableDatas)
			throws Exception {
		createTable(conf, tableName, schema, tableDatas, 1);
	}

	public static void createTable(TajoConf conf, String tableName, Schema schema, String[] tableDatas,
			int numDataFiles) throws Exception {
		TpchTestBase instance = TpchTestBase.getInstance();
		TajoTestingCluster util = instance.getTestingCluster();
